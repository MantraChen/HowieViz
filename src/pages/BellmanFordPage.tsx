...
function PseudocodePanel() {
  const currentLine = useBellmanFordStore(s => s.currentLine ?? 0)
...
function StepsPanel({ steps, clearSteps }: { steps: { time: string; text: string }[]; clearSteps: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — run Bellman-Ford.</p>
      ) : (
...
export function BellmanFordPage() {
  ...
  const store = useBellmanFordStore()
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    ...
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">Handles negative edges; detects negative cycles</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Bellman-Ford</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Shortest paths with negative edges — V-1 relaxation passes</p>
        </div>
