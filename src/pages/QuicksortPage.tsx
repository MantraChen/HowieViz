...
import { useQuicksortStore } from '@/store/quicksortStore'
import { cn } from '@/lib/utils'
import { QuizPanel } from '@/components/QuizPanel'
import { QUICKSORT_QUIZ } from '@/data/quizQuestions'
...
function PseudocodePanel() {
  const currentLine = useQuicksortStore(s => s.currentLine ?? 0)
...
export function QuicksortPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={QUICKSORT_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Quicksort</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Pivot partition — O(n log n) avg, O(n²) worst case</p>
        </div>
