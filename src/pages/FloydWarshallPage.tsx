...
import { FloydWarshallControls } from '@/components/FloydWarshallControls'
import { useFloydWarshallStore } from '@/store/floydWarshallStore'
import { cn } from '@/lib/utils'
import { QuizPanel } from '@/components/QuizPanel'
import { FLOYD_WARSHALL_QUIZ } from '@/data/quizQuestions'

type ModeKey = 'visualize' | 'manual' | 'quiz' | 'compare' | 'embed'

const PSEUDOCODE_LINES = [
  'floydWarshall(G):',
  '  dist[i][j] = edge(i,j) or ∞',
  '  dist[i][i] = 0',
  '  for k = 0 to V-1:',
  '    for i = 0 to V-1:',
  '      for j = 0 to V-1:',
  ...
  '          dist[i][j] = dist[i][k]',
  '                      + dist[k][j]',
  '  return dist  // all-pairs shortest',
]
...
function PseudocodePanel() {
  const currentLine = useFloydWarshallStore(s => s.currentLine ?? 0)
...
function StepsPanel({ steps, clearSteps }: { steps: { time: string; text: string }[]; clearSteps: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — run Floyd-Warshall.</p>
      ) : (
...
export function FloydWarshallPage() {
  ...
  const store = useFloydWarshallStore()
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={FLOYD_WARSHALL_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
          ...
          <tbody>
            {COMPLEXITY_ROWS.map(({ op, time }) => (
              <tr key={op} className="border-b border-[#1e1630] last:border-0">
                ...
                <td className="py-2 text-right font-mono text-[#a78bde] font-medium">{time}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-[10px] text-[#3d2d5a] mt-2">All-pairs; handles negative edges (no neg. cycles)</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Floyd-Warshall</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">All-pairs shortest paths — O(V³) DP over intermediate vertices</p>
        </div>
