...
const COMPLEXITY_ROWS = [
  { op: 'Floyd-Warshall', time: 'O(V³)' },
  { op: 'Space', time: 'O(V²)' },
  { op: 'All-pairs SP', time: 'V² paths' },
]
...
export function FloydWarshallPage() {
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#f0eaf8]">Floyd-Warshall</h1>
        <p className="text-sm text-[#a78bde] mt-1">
          All-pairs shortest paths via dynamic programming. For each intermediate node k, checks if
          routing through k improves the path between every pair (i, j). Watch the distance matrix
          fill in real time.
        </p>
      </div>
