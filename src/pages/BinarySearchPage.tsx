...
const PSEUDOCODE_LINES = [
  'binarySearch(arr, target):  // sorted arr',
  '  lo = 0; hi = n-1',
  '  while lo <= hi:',
  '    mid = (lo + hi) / 2',
...
const COMPLEXITY_ROWS = [
  { op: 'Best',  time: 'O(1)'     },
  { op: 'Avg',   time: 'O(log n)' },
  { op: 'Worst', time: 'O(log n)' },
  { op: 'Space', time: 'O(1)'     },
]

function PseudocodePanel() {
  const currentLine = useBinarySearchStore(s => s.currentLine ?? 0)
          ...
          <tbody>
            {COMPLEXITY_ROWS.map(({ op, time }) => (
              <tr key={op} className="border-b border-[#1e1630] last:border-0">
                ...
                <td className={`py-2 text-right font-mono ${time === 'O(1)' ? 'text-[#d4a8ff] font-bold' : time.includes('log') ? 'text-[#b892e8] font-semibold' : 'text-[#a78bde] font-medium'}`}>{time}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-[10px] text-[#3d2d5a] mt-2">Requires sorted input</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Binary Search</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Halve the search space each step — O(log n) on sorted arrays</p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        <div className="flex-1 min-w-0 overflow-y-auto p-5 space-y-4">
          <div className="max-h-[55vh] overflow-auto">
            <BinarySearchVisualizer elements={store.elements} description={store.description} />
          </div>
      ...
      <div className="flex-none h-9 border-t border-[#2a1f3d] bg-[#0f0b17] px-4 flex items-center gap-2">
        ...
        <span className="text-[11px] text-[#e1d2e9] font-mono truncate">{store.description}</span>
      </div>
