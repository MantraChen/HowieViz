...
function PseudocodePanel() {
  const currentLine = useUFStore(s => s.currentLine ?? 0)
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#2a1f3d]">
              <th className="text-left pb-2 text-[#6b4d8a] font-medium uppercase tracking-[0.06em]">Op</th>
              <th className="text-right pb-2 text-[#6b4d8a] font-medium uppercase tracking-[0.06em]">Amortized</th>
            </tr>
          </thead>
          <tbody>
            {COMPLEXITY_ROWS.map(({ op, time }) => (
              <tr key={op} className="border-b border-[#1e1630] last:border-0">
                ...
                <td className={`py-2 text-right font-mono ${time.includes('α') ? 'text-[#d4a8ff] font-bold' : 'text-[#a78bde] font-medium'}`}>{time}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-[10px] text-[#3d2d5a] mt-2">α(n) = inverse Ackermann, practically O(1)</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Union-Find / Disjoint Set</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Path compression + union by rank — near-O(1) per operation</p>
        </div>
      ...
      <div className="flex-none h-9 border-t border-[#2a1f3d] bg-[#0f0b17] px-4 flex items-center gap-2">
        ...
        <span className="text-[11px] text-[#3d2d5a] font-mono truncate">Union nodes to merge sets, Find to query representative</span>
      </div>
