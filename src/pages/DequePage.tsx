...
import { DEQUE_QUIZ } from '@/data/quizQuestions'
...
const COMPLEXITY_ROWS = [
  ...
  { op: 'Push Back',  time: 'O(1)' },
  { op: 'Pop Front',  time: 'O(1)' },
  { op: 'Pop Back',   time: 'O(1)' },
]

function PseudocodePanel() {
  const currentLine = useDequeStore(s => s.currentLine ?? 0)
...
function StepsPanel() {
  const steps = useDequeStore(s => s.steps)
  const clearSteps = useDequeStore(s => s.clearSteps)
...
export function DequePage() {
  ...
  const statusText = useDequeStore(s => s.statusText)

  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={DEQUE_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <DequeControls mode={activeMode} />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Deque</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Push and pop from both ends in O(1)</p>
        </div>
