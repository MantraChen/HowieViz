...
const COMPLEXITY_ROWS = [
  { op: 'Kahn\'s algorithm', time: 'O(V + E)' },
  { op: 'Process each node', time: 'O(1)' },
  { op: 'Traverse each edge', time: 'O(E)' },
  { op: 'Space', time: 'O(V)' },
]
...
export function TopoSortPage() {
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#f0eaf8]">Topological Sort</h1>
        <p className="text-sm text-[#a78bde] mt-1">
          Linear ordering of vertices in a DAG such that every directed edge u→v has u before v.
          Uses Kahn's algorithm: repeatedly pick nodes with in-degree 0, remove them, and update neighbours.
        </p>
      </div>
