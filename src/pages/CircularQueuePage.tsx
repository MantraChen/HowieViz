...
function PseudocodePanel() {
  const currentLine = useCircularQueueStore(s => s.currentLine ?? 0)
...
function StepsPanel() {
  const steps = useCircularQueueStore(s => s.steps)
  const clearSteps = useCircularQueueStore(s => s.clearSteps)
...
export function CircularQueuePage() {
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Circular Queue</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Fixed-capacity FIFO — tail wraps around to head</p>
        </div>
