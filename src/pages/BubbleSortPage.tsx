...
import { BUBBLE_SORT_QUIZ } from '@/data/quizQuestions'
...
const PSEUDOCODE_LINES = [
  ...
  '// each pass bubbles the max to its final place',
]
...
function complexityClass(time: string): string {
  ...
  if (time === 'O(n)') return 'text-[#b892e8] font-semibold'
  return 'text-[#a78bde] font-medium'
}

function PseudocodePanel() {
  const currentLine = useBubbleSortStore(s => s.currentLine ?? 0)
...
function StepsPanel() {
  const steps = useBubbleSortStore(s => s.steps)
  const clearSteps = useBubbleSortStore(s => s.clearSteps)
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — press Sort.</p>
      ) : (
...
export function BubbleSortPage() {
  ...
  const store = useBubbleSortStore()
  const statusText = store.statusText ?? store.description ?? ''
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={BUBBLE_SORT_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <SortControls
        ...
        onReset={store.reset}
        snaps={store.snaps ?? []}
        snapIndex={store.snapIndex ?? -1}
        onPrepareSnaps={store.prepareSnaps}
        onStepForward={store.stepForward}
        onStepBack={store.stepBack}
        onLoadCustom={store.loadCustom}
      />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      ...
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Bubble Sort</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Adjacent swaps — O(n²) average, O(n) best case</p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-5 space-y-4">
          <div className="max-h-[55vh] overflow-auto">
            <SortVisualizer bars={store.bars} description={statusText} />
          </div>
      ...
      <div className="flex-none h-9 border-t border-[#2a1f3d] bg-[#0f0b17] px-4 flex items-center gap-2">
        ...
        {activeMode === 'manual' && snapsLength > 0 && (
          <span className="text-[11px] text-[#6b4d8a] font-mono flex-none">Step {snapIndex + 1} / {snapsLength}</span>
        )}
        {(activeMode !== 'manual' || snapsLength > 0) && <span className="text-[#3d2d5a] text-[11px] flex-none">·</span>}
