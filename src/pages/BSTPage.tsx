...
function PseudocodePanel() {
  const currentLine = useBSTStore(s => s.currentLine ?? 0)
...
export function BSTPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={BST_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <BSTControls mode={activeMode} />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Binary Search Tree</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">BST property: left &lt; node &lt; right — O(log n) avg</p>
        </div>
