...
import { PrimsVisualizer } from '@/visualizers/PrimsVisualizer'
import { PrimsControls } from '@/components/PrimsControls'
import { usePrimsStore } from '@/store/primsStore'
import { cn } from '@/lib/utils'
import { QuizPanel } from '@/components/QuizPanel'
import { PRIMS_QUIZ } from '@/data/quizQuestions'
...
const COMPLEXITY_ROWS = [
  { op: 'Time (heap)', time: 'O(E log V)' },
  { op: 'Space',       time: 'O(V + E)'   },
]

function PseudocodePanel() {
  const currentLine = usePrimsStore(s => s.currentLine ?? 0)
...
function StepsPanel({ steps, clearSteps }: { steps: { time: string; text: string }[]; clearSteps: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — run Prim's.</p>
      ) : (
...
export function PrimsPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={PRIMS_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">Greedy MST; grows one vertex at a time</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Prim's Algorithm</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Minimum Spanning Tree — grow from a seed, always pick cheapest edge</p>
        </div>
