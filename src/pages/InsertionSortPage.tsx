...
import { INSERTION_SORT_QUIZ } from '@/data/quizQuestions'
...
const PSEUDOCODE_LINES = [
  ...
  '    j -= 1',
  '  arr[j+1] = key',
  '// shifts larger elements right; O(n) best on sorted input',
]
...
function PseudocodePanel() {
  const currentLine = useInsertionSortStore(s => s.currentLine ?? 0)
...
export function InsertionSortPage() {
  ...
  const store = useInsertionSortStore()
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={INSERTION_SORT_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Insertion Sort</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Build sorted prefix — O(n²) average, O(n) best case</p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        <div className="flex-1 min-w-0 overflow-y-auto p-5 space-y-4">
          ...
          <CollapsibleSection title="Steps" open={stepsOpen} onToggle={() => setStepsOpen(v => !v)}>
            <StepsPanel steps={store.steps ?? []} clearSteps={store.clearSteps ?? (() => {})} />
          </CollapsibleSection>
        </div>
