...
import { ARRAY_QUIZ } from '@/data/quizQuestions'
...
const COMPLEXITY_ROWS = [
  { op: 'Access',        time: 'O(1)' },
  { op: 'Push',          time: 'O(1) amortized' },
  { op: 'Pop',           time: 'O(1)' },
  { op: 'Insert (mid)',  time: 'O(n)' },
  { op: 'Delete (mid)',  time: 'O(n)' },
  { op: 'Search',        time: 'O(n)' },
]

function PseudocodePanel() {
  const currentLine = useArrayStore(s => s.currentLine ?? 0)
...
function StepsPanel() {
  const steps = useArrayStore(s => s.steps)
  const clearSteps = useArrayStore(s => s.clearSteps)
...
export function ArrayPage() {
  ...
  const statusText = useArrayStore(s => s.statusText)

  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={ARRAY_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <ArrayControls mode={activeMode} />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Array</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Dynamic array — contiguous memory, indexed access</p>
        </div>
