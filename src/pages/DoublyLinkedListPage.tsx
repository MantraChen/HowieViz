...
import { DoublyLinkedListControls } from '@/components/DoublyLinkedListControls'
import { useDLLStore } from '@/store/doublyLinkedListStore'
import { cn } from '@/lib/utils'
import { QuizPanel } from '@/components/QuizPanel'
import { DOUBLY_LINKED_LIST_QUIZ } from '@/data/quizQuestions'
...
const COMPLEXITY_ROWS = [
  ...
  { op: 'Delete (with ref)', time: 'O(1)' },
  { op: 'Search',            time: 'O(n)' },
]

function PseudocodePanel() {
  const currentLine = useDLLStore(s => s.currentLine ?? 0)
...
function StepsPanel() {
  const steps = useDLLStore(s => s.steps)
  const clearSteps = useDLLStore(s => s.clearSteps)
...
export function DoublyLinkedListPage() {
  ...
  const statusText = useDLLStore(s => s.statusText)

  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={DOUBLY_LINKED_LIST_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <DoublyLinkedListControls mode={activeMode} />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Doubly Linked List</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Bidirectional links — O(1) insert/delete at ends</p>
        </div>
