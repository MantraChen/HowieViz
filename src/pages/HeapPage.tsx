...
import { HEAP_QUIZ } from '@/data/quizQuestions'
...
const PSEUDOCODE_LINES = [
  'insert(val):',
  '  add to end of array',
  '  bubbleUp: while val < parent:',
  '    swap with parent; move up',
  'extractMin():',
  '  min = root; replace root with last',
  '  siftDown: while val > smaller child:',
  '    swap with smaller child; move down',
  'parent(i) = (i-1)/2',
...
function PseudocodePanel() {
  const currentLine = useHeapStore(s => s.currentLine ?? 0)
...
export function HeapPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={HEAP_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <HeapControls mode={activeMode} />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Binary Heap (Min-Heap)</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Complete binary tree — parent ≤ children, O(log n) insert/extract</p>
        </div>
