...
import { QUEUE_QUIZ } from '@/data/quizQuestions'
...
function PseudocodePanel() {
  const currentLine = useQueueStore(s => s.currentLine ?? 0)
...
function StepsPanel() {
  const steps = useQueueStore(s => s.steps)
  const clearSteps = useQueueStore(s => s.clearSteps)
...
export function QueuePage() {
  ...
  const statusText = useQueueStore(s => s.statusText)

  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={QUEUE_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      <QueueControls mode={activeMode} />
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Queue</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">FIFO — enqueue to back, dequeue from front in O(1)</p>
        </div>
