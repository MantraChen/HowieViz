...
import { MERGE_SORT_QUIZ } from '@/data/quizQuestions'
...
const PSEUDOCODE_LINES = [
  ...
  '  merge(arr, lo, mid, hi)',
  'merge: copy halves to temp arrays',
  '  then interleave back by comparing heads',
]
...
function complexityClass(time: string): string {
  ...
  if (time.includes('log') || time === 'O(n)') return 'text-[#b892e8] font-semibold'
  return 'text-[#a78bde] font-medium'
}

function PseudocodePanel() {
  const currentLine = useMergeSortStore(s => s.currentLine ?? 0)
...
export function MergeSortPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={MERGE_SORT_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Merge Sort</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Divide & conquer — O(n log n) all cases, O(n) space</p>
        </div>
