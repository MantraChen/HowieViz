...
import { FENWICK_QUIZ } from '@/data/quizQuestions'
...
const COMPLEXITY_ROWS = [
  { op: 'Prefix Query', time: 'O(log n)' },
  { op: 'Point Update', time: 'O(log n)' },
  { op: 'Build',        time: 'O(n log n)' },
]
...
export function FenwickPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={FENWICK_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">lowbit(i) = i &amp; (−i)</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Fenwick Tree / BIT</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Binary Indexed Tree — O(log n) prefix query & point update</p>
        </div>
