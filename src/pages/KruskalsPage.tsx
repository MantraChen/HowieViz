...
const COMPLEXITY_ROWS = [
  { op: 'Time',  time: 'O(E log E)' },
  { op: 'Space', time: 'O(V)'       },
]

function PseudocodePanel() {
  const currentLine = useKruskalsStore(s => s.currentLine ?? 0)
...
function StepsPanel({ steps, clearSteps }: { steps: { time: string; text: string }[]; clearSteps: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — run Kruskal's.</p>
      ) : (
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    ...
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">Uses Union-Find; sort dominates runtime</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Kruskal's Algorithm</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Minimum Spanning Tree — sort edges, union-find cycle detection</p>
        </div>
