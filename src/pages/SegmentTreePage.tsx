...
import { SegmentTreeControls } from '@/components/SegmentTreeControls'
import { useSTStore } from '@/store/segmentTreeStore'
import { cn } from '@/lib/utils'
import { QuizPanel } from '@/components/QuizPanel'
import { SEGMENT_TREE_QUIZ } from '@/data/quizQuestions'
...
const PSEUDOCODE_LINES = [
  ...
  '  mid = (lo + hi) / 2',
  '  build(arr, left(node), lo, mid)',
  '  build(arr, right(node), mid+1, hi)',
  '  tree[node] = tree[left] + tree[right]',
  'query(l, r, node, lo, hi):',
  '  if [lo,hi] ⊆ [l,r]: return tree[node]',
  '  if [lo,hi] ∩ [l,r] = ∅: return 0',
  '  return query(left) + query(right)',
  'update(i, val): update path i → root',
]
...
function PseudocodePanel() {
  const currentLine = useSTStore(s => s.currentLine ?? 0)
...
function StepsPanel({ steps, clearSteps }: { steps: { time: string; text: string }[]; clearSteps: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — use controls.</p>
      ) : (
...
export function SegmentTreePage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={SEGMENT_TREE_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
          ...
          <tbody>
            {COMPLEXITY_ROWS.map(({ op, time }) => (
              <tr key={op} className="border-b border-[#1e1630] last:border-0">
                ...
                <td className={`py-2 text-right font-mono ${time.includes('log') ? 'text-[#b892e8] font-semibold' : 'text-[#a78bde] font-medium'}`}>{time}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Segment Tree</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Range aggregation — O(n) build, O(log n) query & update</p>
        </div>
