...
import { LINEAR_SEARCH_QUIZ } from '@/data/quizQuestions'
...
function PseudocodePanel() {
  const currentLine = useLinearSearchStore(s => s.currentLine ?? 0)
...
export function LinearSearchPage() {
  ...
  const store = useLinearSearchStore()
  const isAnimating = (store as any).isAnimating ?? false

  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={LINEAR_SEARCH_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Linear Search</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Sequential scan — O(n) worst case, no sorted requirement</p>
        </div>
