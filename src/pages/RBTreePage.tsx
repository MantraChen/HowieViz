...
const PSEUDOCODE_LINES = [
  ...
  'RB properties:',
  '  root = black',
  '  red → two black children',
  '  all paths to NIL: equal black-height',
]
...
function PseudocodePanel() {
  const currentLine = useRBTreeStore(s => s.currentLine ?? 0)
  ...
  const rightPanelContent = (
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">NIL leaves shown as black squares</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Red-Black Tree</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Self-balancing BST with color invariants — O(log n) guaranteed</p>
        </div>
