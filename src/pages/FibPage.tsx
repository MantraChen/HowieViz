...
import { FIBONACCI_QUIZ } from '@/data/quizQuestions'
import { useFibStore } from '@/store/fibStore'
...
const COMPLEXITY_ROWS = [
  ...
  { op: 'Memo',   time: 'O(n)',   space: 'O(n)' },
  { op: 'Bottom-up', time: 'O(n)', space: 'O(1)' },
]

function PseudocodePanel() {
  const currentLine = useFibStore(s => s.currentLine ?? 0)
...
export function FibPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={FIBONACCI_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#2a1f3d]">
              <th className="text-left pb-2 text-[#6b4d8a] font-medium uppercase tracking-[0.06em]">Strategy</th>
              <th className="text-center pb-2 text-[#6b4d8a] font-medium uppercase tracking-[0.06em]">Time</th>
          ...
          <tbody>
            {COMPLEXITY_ROWS.map(({ op, time, space }) => (
              <tr key={op} className="border-b border-[#1e1630] last:border-0">
                ...
                <td className={`py-2 text-center font-mono ${time.includes('2') ? 'text-[#6b4d8a]' : 'text-[#b892e8] font-semibold'}`}>{time}</td>
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Fibonacci DP</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Naive vs memoization vs tabulation — overlapping subproblems</p>
        </div>
      ...
      <div className="flex-none h-9 border-t border-[#2a1f3d] bg-[#0f0b17] px-4 flex items-center gap-2">
        ...
        <span className="text-[11px] text-[#3d2d5a] font-mono truncate">Select n and a strategy to visualize Fibonacci</span>
      </div>
