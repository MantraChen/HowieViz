...
import { DIJKSTRA_QUIZ } from '@/data/quizQuestions'

type ModeKey = 'visualize' | 'manual' | 'quiz' | 'compare' | 'embed'

const PSEUDOCODE_LINES = [
  'dijkstra(G, src):',
  '  dist[src] = 0; dist[v] = ∞ for all others',
...
const COMPLEXITY_ROWS = [
  { op: 'Overall',    time: 'O((V+E) log V)' },
  { op: 'Relaxation', time: 'O(E log V)'      },
  { op: 'Space',      time: 'O(V + E)'        },
]

function PseudocodePanel() {
  const currentLine = useDijkstraStore(s => s.currentLine ?? 0)
...
function StepsPanel({ steps, clearSteps }: { steps: { time: string; text: string }[]; clearSteps: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {steps.length === 0 ? (
        <p className="text-xs text-[#3d2d5a] italic font-mono">No steps yet — run Dijkstra.</p>
      ) : (
...
export function DijkstraPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={DIJKSTRA_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">Non-negative edge weights only</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Dijkstra's Algorithm</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Single-source shortest paths — greedy min-heap relaxation</p>
        </div>
