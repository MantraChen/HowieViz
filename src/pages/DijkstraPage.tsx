...
const COMPLEXITY_ROWS = [
  { op: "Dijkstra's", time: 'O((V+E) log V)' },
  { op: 'Add to queue', time: 'O(log V)' },
  { op: 'Relax edge', time: 'O(log V)' },
]
...
export function DijkstraPage() {
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#f0eaf8]">Dijkstra's Algorithm</h1>
        <p className="text-sm text-[#a78bde] mt-1">
          Shortest path on a weighted directed graph. Greedily settles the closest unvisited node each
          step, relaxing outgoing edges. The distance table updates in real time.
        </p>
      </div>
