...
import { LCS_QUIZ } from '@/data/quizQuestions'
...
const PSEUDOCODE_LINES = [
  ...
  '        dp[i][j] = max(dp[i-1][j],',
  '                       dp[i][j-1])',
  '  // backtrack dp to reconstruct LCS',
  '  return dp[m][n]',
]
...
function PseudocodePanel() {
  const currentLine = useLCSStore(s => s.currentLine ?? 0)
...
export function LCSPage() {
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    <QuizPanel questions={LCS_QUIZ} onComplete={() => setActiveMode('visualize')} />
  ) : (
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">m, n = string lengths</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Longest Common Subsequence</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Classic DP — O(m×n) table fill, backtrack to reconstruct</p>
        </div>
      ...
      <div className="flex-none h-9 border-t border-[#2a1f3d] bg-[#0f0b17] px-4 flex items-center gap-2">
        ...
        <span className="text-[11px] text-[#3d2d5a] font-mono truncate">Enter two strings and compute LCS</span>
      </div>
