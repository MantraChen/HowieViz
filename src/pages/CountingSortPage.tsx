...
function PseudocodePanel() {
  const currentLine = useCountingSortStore(s => s.currentLine ?? 0)
...
export function CountingSortPage() {
  ...
  const store = useCountingSortStore()
  ...
  const rightPanelContent = activeMode === 'quiz' ? (
    ...
    <div className="space-y-4 p-4">
      ...
      <CollapsibleSection title="Complexity" open={complexityOpen} onToggle={() => setComplexityOpen(v => !v)}>
        ...
        <p className="text-[10px] text-[#3d2d5a] mt-2">n = array size, k = value range</p>
      </CollapsibleSection>
    </div>
  )
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Counting Sort</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">Non-comparison sort — O(n + k) time, O(k) space</p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        <div className="flex-1 min-w-0 overflow-y-auto p-5 space-y-4">
          <div className="max-h-[55vh] overflow-auto">
            <CountingSortVisualizer step={store.step} description={statusText} />
          </div>
