...
const PSEUDOCODE_LINES = [
  ...
  '  for pass = 0 to d-1:      // LSD → MSD',
  '    stably sort arr by digit[pass]',
  '    (counting sort on digits 0..9)',
  '// d passes × O(n + 10) = O(d × n)',
  '// no element comparisons — sort by digits',
]
...
function PseudocodePanel() {
  const currentLine = useRadixSortStore(s => s.currentLine ?? 0)
    ...
    <div className="h-full flex flex-col bg-[#090710]">
      <div className="flex-none flex items-center justify-between px-5 h-14 border-b border-[#2a1f3d] bg-[#0f0b17]">
        <div className="flex flex-col justify-center">
          <h1 className="text-base font-bold text-[#f0eaf8] leading-tight">Radix Sort</h1>
          <p className="text-[11px] text-[#6b4d8a] leading-tight">LSD digit-by-digit — O(d × n) time, no comparisons</p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        <div className="flex-1 min-w-0 overflow-y-auto p-5 space-y-4">
          <div className="max-h-[55vh] overflow-auto">
            <RadixSortVisualizer step={store.step} description={statusText} />
          </div>
