...
function generateSteps(input: number[]): SortStep[] {
  ...
  function snap(hl: (i: number) => BarHighlight, desc: string) {
    steps.push({ bars: a.map((v, i) => ({ value: v, highlight: hl(i) })), description: desc })
  }
  ...
  function merge(lo: number, mid: number, hi: number) {
    ...
    snap(rangeHl(lo, mid, hi), `Merge [${lo}..${mid}] and [${mid + 1}..${hi}]`)

    let li = 0, ri = 0, k = lo

    while (li < left.length && ri < right.length) {
      snap(rangeHl(lo, mid, hi), `Compare ${left[li]} and ${right[ri]}`)
    ...
    snap(i => (i >= lo && i <= hi ? 'sorted' : 'default'), `Merged [${lo}..${hi}]`)
  }

  function mergeSort(lo: number, hi: number) {
    if (lo >= hi) return
    const mid = Math.floor((lo + hi) / 2)
    snap(rangeHl(lo, mid, hi), `Split [${lo}..${hi}] → [${lo}..${mid}] | [${mid + 1}..${hi}]`)
    mergeSort(lo, mid)
    mergeSort(mid + 1, hi)
    merge(lo, mid, hi)
  }
