...
export const useHeapStore = create<HeapStore>((set, get) => ({
  ...
  reset: () => {
    ...
    const gen = ++animGen
    if (heap.length === 0) {
      set({ heap: DEFAULT_HEAP.map(n => ({ ...n, id: nanoid(), highlight: 'default' as const })), isAnimating: false })
      return
    }
    set({ isAnimating: true, heap: heap.map(n => ({ ...n, highlight: 'deleted' as const })) })
    const t = setTimeout(() => {
      if (animGen !== gen) return
      set({ isAnimating: false, heap: DEFAULT_HEAP.map(n => ({ ...n, id: nanoid(), highlight: 'default' as const })) })
    }, 650)
    animTimers.push(t)
  },
  ...
  peek: () => {
    ...
    const t = setTimeout(() => {
      if (animGen !== gen) return
      set(s => ({ isAnimating: false, heap: s.heap.map(n => ({ ...n, highlight: 'default' })) }))
    }, delay * 3)
    animTimers.push(t)
  },
  ...
  extractMin: () => {
    ...
    if (heap.length === 1) {
      ...
      // Move last element to root, remove last
