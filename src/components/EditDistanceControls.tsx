...
export function EditDistanceControls() {
  const { str1Input, str2Input, speed, isAnimating, setStr1Input, setStr2Input, setSpeed, solve, reset } = useEditDistanceStore()

  const canSolve = str1Input.trim().length > 0 && str2Input.trim().length > 0 && !isAnimating

  const inputClass =
    'w-full h-10 px-3 rounded-md bg-[#1a1428] border border-[#2a1f3d] text-sm text-[#f0eaf8] placeholder:text-[#3d2d5a] focus:outline-none focus:border-[#b892e8] transition-colors disabled:opacity-40 font-mono'

  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <label className="text-sm text-[#a78bde]">String 1 (max 10)</label>
        <input type="text" value={str1Input} onChange={e => setStr1Input(e.target.value)}
          placeholder="kitten" disabled={isAnimating} className={inputClass} />
      </div>
      <div className="space-y-1.5">
        <label className="text-sm text-[#a78bde]">String 2 (max 10)</label>
        <input type="text" value={str2Input} onChange={e => setStr2Input(e.target.value)}
          placeholder="sitting" disabled={isAnimating} className={inputClass} />
      </div>
