...
export function FibControls() {
  const { nInput, speed, isAnimating, setNInput, setSpeed, compute, reset } = useFibStore()
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <label className="text-sm text-[#a78bde]">n (0–15)</label>
        <input
          type="number"
          min={0}
          max={15}
          ...
          onKeyDown={e => e.key === 'Enter' && valid && !isAnimating && compute()}
          placeholder="e.g. 6"
      ...
      <div className="grid grid-cols-2 gap-2">
        <Btn onClick={() => valid && !isAnimating && compute()} disabled={!valid || isAnimating} variant="primary">Compute</Btn>
