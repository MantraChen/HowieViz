...
export function DijkstraControls() {
  const {
    nodes, isAnimating, speed, startNode,
    setSpeed, setStartNode, runDijkstra, reset,
  } = useDijkstraStore()
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-2">
        <label className="text-xs font-medium text-[#a78bde] uppercase tracking-[0.06em]">Dijkstra's</label>
        <div className="space-y-2">
          ...
          <Btn onClick={() => !isAnimating && runDijkstra(startNode)} disabled={isAnimating} variant="primary">
            Run Dijkstra
          </Btn>
        </div>
      </div>
