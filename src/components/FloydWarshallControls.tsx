...
export function FloydWarshallControls() {
  const { isAnimating, speed, setSpeed, run, reset } = useFloydWarshallStore()

  return (
    <div className="space-y-5">
      <div className="space-y-2">
        <label className="text-xs font-medium text-[#a78bde] uppercase tracking-[0.06em]">Floyd-Warshall</label>
      ...
      <div className="text-xs text-[#6b4d8a] space-y-1 pt-2 border-t border-[#2a1f3d]">
        <p>Floyd-Warshall computes shortest paths between <span className="text-[#a78bde]">all pairs</span> of nodes.</p>
        <p>Watch the distance matrix update as each intermediate node k is considered.</p>
      </div>
    </div>
  )
}
