import { useBinarySearchStore } from '@/store/binarySearchStore'
import { cn } from '@/lib/utils'

export function BinarySearchControls() {
  const { target, speed, isAnimating, isDone, array, setTarget, setSpeed, search, randomize, reset } =
    useBinarySearchStore()

  const parsed = parseInt(target, 10)
  const hasTarget = !isNaN(parsed)

  const inputClass =
    'w-full h-10 px-3 rounded-md bg-[#1a1428] border border-[#2a1f3d] text-sm text-[#f0eaf8] placeholder:text-[#3d2d5a] focus:outline-none focus:border-[#b892e8] transition-colors disabled:opacity-40'

  function onKey(e: React.KeyboardEvent) {
    if (e.key === 'Enter' && hasTarget && !isAnimating) search()
  }

  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <label className="text-sm text-[#a78bde]">Target Value</label>
        <input
          type="number"
          value={target}
          onChange={e => setTarget(e.target.value)}
          onKeyDown={onKey}
          placeholder={`e.g. ${array[Math.floor(array.length / 2)] ?? 42}`}
          disabled={isAnimating}
          className={inputClass}
        />
      </div>

      <Btn
        onClick={() => hasTarget && !isAnimating && search()}
        disabled={!hasTarget || isAnimating}
        variant="primary"
      >
        {isAnimating ? 'Searching…' : isDone ? 'Search Again' : 'Search'}
      </Btn>

      <div className="grid grid-cols-2 gap-2">
        <Btn onClick={() => !isAnimating && randomize()} disabled={isAnimating} variant="neutral">
          Randomize
        </Btn>
        <Btn onClick={() => !isAnimating && reset()} disabled={isAnimating} variant="neutral">
          Reset
        </Btn>
      </div>

      <div className="flex items-center gap-2 pt-1">
        <span className="text-xs text-[#6b4d8a]">Speed:</span>
        {(['slow', 'normal', 'fast'] as const).map(s => (
          <button
            key={s}
            onClick={() => setSpeed(s)}
            className={cn(
              'text-xs w-14 h-7 rounded capitalize transition-all duration-200',
              speed === s
                ? 'bg-[#744cae] text-white font-medium shadow-[0_0_12px_rgba(180,130,232,0.4)]'
                : 'text-[#6b4d8a] hover:text-[#a78bde]',
            )}
          >
            {s}
          </button>
        ))}
      </div>
    </div>
  )
}

interface BtnProps {
  onClick: () => void
  disabled: boolean
  variant: 'primary' | 'neutral'
  children: React.ReactNode
}

function Btn({ onClick, disabled, variant, children }: BtnProps) {
  const styles = {
    primary: 'bg-[#744cae]/20 hover:bg-[#744cae] border-[#744cae]/50 text-[#d4a8ff] hover:text-white hover:shadow-[0_0_12px_rgba(180,130,232,0.4)]',
    neutral: 'bg-transparent hover:bg-[#1e1630] border-[#2a1f3d] hover:border-[#3d2d5a] text-[#6b4d8a] hover:text-[#a78bde]',
  }
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={cn(
        'h-10 px-4 w-full flex items-center justify-center rounded-md border text-sm font-medium transition-all duration-200',
        styles[variant],
        disabled && 'opacity-30 cursor-not-allowed pointer-events-none',
      )}
    >
      {children}
    </button>
  )
}
