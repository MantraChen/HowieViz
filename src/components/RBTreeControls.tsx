...
export function RBTreeControls() {
  const { inputValue, speed, isAnimating, setInputValue, setSpeed, insert, search, reset } = useRBTreeStore()
  ...
  return (
    <div className="space-y-4">
      ...
      <Btn onClick={() => !isAnimating && reset()} disabled={isAnimating} variant="neutral" full>Reset</Btn>
...
function Btn({ onClick, disabled, variant, children, full }: BtnProps) {
  const styles = {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={cn('h-10 px-4 flex items-center justify-center rounded-md border text-sm font-medium transition-all duration-200',
        full ? 'w-full' : 'w-full',
        styles[variant],
