...
export function LinearSearchControls() {
  const { array, target, speed, isAnimating, isDone, setTarget, setSpeed, search, randomize, reset } =
    useLinearSearchStore()

  const parsed   = parseInt(target, 10)
  const hasTarget = !isNaN(parsed)
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <label className="text-sm text-[#a78bde]">Target Value</label>
        <input
          type="number"
          value={target}
          onChange={e => setTarget(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && hasTarget && !isAnimating && search()}
          placeholder={`e.g. ${array[Math.floor(array.length / 3)] ?? 42}`}
          disabled={isAnimating}
          className={inputClass}
        />
      </div>

      <Btn onClick={() => hasTarget && !isAnimating && search()} disabled={!hasTarget || isAnimating} variant="primary">
        {isAnimating ? 'Searching…' : isDone ? 'Search Again' : 'Search'}
      </Btn>

      <div className="grid grid-cols-2 gap-2">
        <Btn onClick={() => !isAnimating && randomize()} disabled={isAnimating} variant="neutral">Randomize</Btn>
