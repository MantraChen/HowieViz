...
export function KruskalsControls() {
  const { isAnimating, speed, setSpeed, run, reset } = useKruskalsStore()

  return (
    <div className="space-y-5">
      <div className="space-y-2">
        <label className="text-xs font-medium text-[#a78bde] uppercase tracking-[0.06em]">Kruskal's MST</label>
      ...
      <div className="text-xs text-[#6b4d8a] space-y-1 pt-2 border-t border-[#2a1f3d]">
        <p>Processes edges in <span className="text-[#a78bde]">sorted order</span> by weight.</p>
        <p>Uses <span className="text-[#a78bde]">Union-Find (DSU)</span> to detect cycles.</p>
        <p>Red edges would create a cycle and are rejected.</p>
      </div>
    </div>
  )
}
